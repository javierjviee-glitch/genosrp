...
function ENT:PhysicsCollide(data, physobj)
  ...
  if not IsValid(ptable) or not IsValid(data.HitEntity) then return end

  local hitData = {
    hitEntity = data.HitEntity,
    hitPos = math.floor(data.HitPos[3]),
    speed = data.Speed,
    oldVelocity = data.OurOldVelocity:Length(),
    theirOldVelocity = data.TheirOldVelocity:Length(),
    deltaTime = data.DeltaTime,
    hitNormal = data.HitNormal,
    physobj = physobj,
    tablePosZ = ptable:GetPos()[3],
    poolPosZ = ptable:GetPos()[3]
  }
  ...
  timer.Simple(0, function()
    ...
    local Speed, HitEnt, hitpos = hitData.speed, hitData.hitEntity, hitData.hitPos
    local poolPosZ = hitData.poolPosZ

    if not ptable.WaitBallsToStop and hitpos >= poolPosZ + 33.9 then
      return physobj:SetVelocity(Vector(0, 0, 0))
    end

    local NewVelocity = physobj:GetVelocity()
    local OldVelocity = hitData.oldVelocity
    local TheirOldVelocity = hitData.theirOldVelocity
    ...
    if HitEnt:GetClass() == "billiard_ball" then
      ...
      if hitData.deltaTime > 0.07 and not ptable.Foul then
        if Speed <= 5 then
      ...
      if OldVelocity > TheirOldVelocity then
        loss = 1 - math.abs(hitData.hitNormal:DotProduct(data.OurOldVelocity:GetNormal()))
