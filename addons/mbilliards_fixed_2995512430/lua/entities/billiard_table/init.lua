...
function ENT:Initialize()
	...
	for i = 1, 6 do
		self.HoleAttachmentIDs[i] = self:LookupAttachment(string.format("hole%02d", i))
	end
	self:GetPhysicsObject():SetMaterial("billiard_table")
	-- Sincronizar el jugador actual con el cliente
	self:SetNWInt("CurrentPlayerID", 0)
end
...
function ENT:UsePressed(ply, caller)
	...
	if self.PlayerData[1] ~= nil then
		...
		if self.PlayerData[2] ~= nil then
			b_error = "La mesa de billar esta llena."
		elseif self.Training then
			b_error = "La mesa esta en modo entrenamiento."
		elseif self:IsPlayerInList(ply:UniqueID()) then
			b_error = "Ya has enviado una solicitud."
		end
		...
		umsg.End()
		umsg.Start("billiard_scrMsg", ply)
		umsg.String("Solicitud enviada")
		umsg.String("Espera la respuesta del otro jugador.")
		umsg.End()
	...
	-- The first player starts the game; additional players use the request list above.
...
function ENT:Think()
	local now = CurTime()
	self:NextThink(now + (self.WaitBallsToStop and 0.05 or 0.2))
	
	-- Sincronizar el jugador actual con los clientes
	local currentPlayer = self:GetTurnPlayer()
	if IsValid(currentPlayer) then
		self:SetNWInt("CurrentPlayerID", currentPlayer:UniqueID())
	else
		self:SetNWInt("CurrentPlayerID", 0)
	end
	
	-- If there are no balls, no need to proceed  xD
	local cueBall = self:GetTableEnt()
	if not IsValid(cueBall) then return true end

	-- We'll check if the "round" time exists and if has expired...
	if self.Timer ~= nil and not self.WaitBallsToStop and self.Timer - now <= 0 then
		-- if so, next player turn...
		local turnPlayer = self:GetTurnPlayer()
		if IsValid(turnPlayer) then
			turnPlayer:BilliardSpectate()
		end
		self.NextPlayer = nil
		self.Foul = true
	...
	if self.ABMethod == 2 and self.GameType ~= BILLIARD_GAMETYPE_CARAMBOL then
		...
		for i = 1, 6 do
			pckt_ents = nil
			pckt_att = self:GetAttachment(self.HoleAttachmentIDs[i])
			if pckt_att then
				pckt_ents = ents.FindInSphere(pckt_att.Pos, 0.8)
			end
			if pckt_ents and next(pckt_ents) then
				for k, v in pairs(pckt_ents) do
					if v:GetClass() == "billiard_ball" and v.BilliardTableID == self.ID then
	...
	for k, v in pairs(self.Balls) do
		...
		if IsValid(physobj) and physobj:GetVelocity():Length2DSqr() > 0.03 then return true end
	end
...
function ENT:StartBilliardGame()
	...
	for i = 0, tmpDat.c do
		...
		if i ~= 0 and i < 7 and self.GameType ~= BILLIARD_GAMETYPE_CARAMBOL then
...
function ENT:RulesParser()
	...
	if self.OpenBreakShot and self.GameType == BILLIARD_GAMETYPE_8BALL and not self:PocketedBall("8-Ball") then
	...
	if self.GameType == BILLIARD_GAMETYPE_8BALL then
		local pocketedBlack = self:PocketedBall("8-Ball")
		local pocketedCue = self:PocketedBall("CueBall")

		-- The 8-ball always ends the game. It is only a win after the
		-- shooter's group has been cleared, and a scratch is always a loss.
		if pocketedBlack then
			if pocketedCue then
				return self:EndBilliardGame(true, "Has perdido porque has metido la negra y la blanca.")
			end

			if cObjective == "OpenTable" then
				if not self:BallTypeExist("Solids") and not self:BallTypeExist("Stripes") then
					return self:EndBilliardGame()
				end
			else
				local ownBallsRemain
				if ply.BilliardTeam == "Solids" or ply.BilliardTeam == "Stripes" then
					ownBallsRemain = self:BallTypeExist(ply.BilliardTeam)
				else
					ownBallsRemain = cObjective ~= "8-Ball"
				end

				if not ownBallsRemain then
					return self:EndBilliardGame()
				end
			end

			return self:EndBilliardGame(true, "Has perdido porque has metido la negra antes de quitar todas tus bolas.")
		end

		if cObjective == "OpenTable" then
			-- Foul if player pockets the Cueball, 8-Ball or pocketes both Stripe and Solid balls
			if self:PocketedBall("Solids") and self:PocketedBall("Stripes") or pocketedCue then
				self.Foul = true
				self.BehindHeadString = false
		...
		elseif cObjective ~= "OpenTable" and cObjective ~= "8-Ball" then
			...
			if self.FirstHitBall ~= cObjective or pocketedCue or self:PocketedBall(wobjective) then
				self.Foul = true
				self.BehindHeadString = false
...
function ENT:EndRound()
	local ply = self:GetTurnPlayer()
	if not IsValid(ply) then return self:EndBilliardGame() end
	...
	local o, pl, ball, cObjective = 1, nil, nil, nil
	...
	if self.GameType == BILLIARD_GAMETYPE_SNOOKER and not self:BallTypeExist("Red") then
		for k, v in pairs(self.Balls) do
			if v.BallType ~= "CueBall" and v.DontRemove then
				v.DontRemove = nil
			end
		end
	...
	if self.ThreeCFouls then
		...
		if self.Foul then
			...
			if self.PlayerData[id].Fouls == 2 then
				umsg.Start("billiard_scrMsg", ply)
				umsg.String("Aviso")
				umsg.String("Ya has cometido 2 faltas.\nOtra falta hara que pierdas la partida.")
				umsg.End()
	...
	if self.Foul then
		...
		local foulTitle = "FALTA"
		local foulReason = fouls or "Se ha cometido una falta."
		if self:PocketedBall("CueBall") then
			foulTitle = "FALTA: BLANCA"
			foulReason = "Has metido la blanca. Es falta."
		end

		if self.BehindHeadString and self.GameType == BILLIARD_GAMETYPE_SNOOKER then
			umsg.String("FALTA - BLANCA EN LA ZONA D")
		else
			umsg.String(foulTitle)
		end

		umsg.String(foulReason)
		umsg.End()
...
function ENT:PocketedBall(bType)
	if self.GameType == BILLIARD_GAMETYPE_SNOOKER then
		...
		if not bType or bType == "Color" or string.match(bType, "(%-Except)$") then
			...
			for i = 1, 7 do
				if not (exclude and tmpDat[i] == exclude or bType == "Color" and i == 1) then
...
function ENT:ResetBallPos()
	...
	elseif self.GameType == BILLIARD_GAMETYPE_SNOOKER and self:BallTypeExist("Red") then
		...
		for i = 1, 6 do
			if self:PocketedBall(tmpDat[i]) then
...
function ENT:EndBilliardGame(inv, reason)
	local soloPlayer = not self.PlayerData[2]
	local currentPlayer = self:GetTurnPlayer()
	reason = reason or "Has perdido la partida."

	if soloPlayer and IsValid(currentPlayer) then
		umsg.Start("billiard_scrMsg", currentPlayer)
		umsg.String(inv and "Derrota" or "Victoria")
		umsg.String(inv and reason or "Has ganado la partida.")
		umsg.End()
	end
	...
	self:ClearPlayers()
	self:ResetVars()
	hook.Call("OnBilliardGameEnd", nil, self, winner, loser, reason)
end
...
function ENT:GetTableEnt(nm)
	...
	elseif string.match(nm, "(%-Ball)$") then
		local ballID = tonumber(string.gsub(nm, "-Ball", ""))

		return self.Balls[ballID]
	else
...
function ENT:SetPlayerObjective(ply, objective)
	...
	if objective == "OpenTable" then
		ply.BilliardTeam = nil
	elseif objective == "Solids" or objective == "Stripes" then
		ply.BilliardTeam = objective
	end
...
function ENT:ClearEnts()
	...
	for k, v in pairs(self.EntsData) do
		...
		if ent then
			if k == "Cue" and not self.FPerson then
...
function ENT:StripPlayerWeapons(plyid)
	...
	if _G.ix then return end

	self.PlayerData[plyid] = self.PlayerData[plyid] or {}
...
function ENT:RestorePlayerWeapons(plyid)
	...
	if not self.PlayerData[plyid] or not IsValid(ply) then return end
	if _G.ix then return end

	for k, v in pairs(self.PlayerData[plyid]["Weapons"] or {}) do
		ply:Give(v.class)
