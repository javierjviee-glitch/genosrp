...
function mEntity:GetBilliardTable()
  if not self.BilliardTableID then return end

  local tableEntity = ents.GetByIndex(self.BilliardTableID)
...
function mEntity:BilliardSpectate(c_b, ent1, ent2)
  ...
  local ent2ID = ent2 and ent2:EntIndex() or "nil"
  local sID = string.format("spec_%s-%s", ent1:EntIndex(), ent2ID)
...
function CBilliardQuitGame(ptable, plyid)
  ...
  if ptable.PlayerData[2] then
    for i = 1, 2 do
      if ptable.PlayerData[i]["ID"] == plyid then
        ptable.Turn = 1
...
function BilliardPlayerDisconnected(ent)
  if IsValid(ent) and ent:IsPlayer() and ent.BilliardTableID and ent.bpUniqueID then
...
hook.Add("PlayerLoadout", "CBilliardPlayerLoadout", function(ply)
  ...
  if not IsValid(ptable) then return end
  if not ply.BilliardTableID or ply.BilliardTableID ~= ptable.ID then return end
...
hook.Add("OnBilliardGameEnd", "EndingGameBilliard", function(ptable, winner, loser, reason)
  if IsValid(winner) then
    umsg.Start("billiard_scrMsg", winner)
    umsg.String("Victoria")
    umsg.String("Enhorabuena, has ganado la partida.")
    umsg.End()
  end
  ...
  if IsValid(loser) then
    ...
    umsg.String("Derrota")
    umsg.String(reason or "Has perdido la partida.")
    umsg.End()
  end
end)
...
hook.Add("Move", "CBilliardMove", function(ply, movedata)
  ...
  if ptable then
    ...
    if b_bool and not ply.WarnAreaSent then
      ...
      umsg.String("Aviso")
      umsg.String("Tienes 5 segundos para volver a la zona de juego\no perderas la partida.")
      umsg.End()
        ...
        if ply:KeyDown(IN_SPEED) and b_bool or ply:KeyDown(IN_SPEED) and ptable.Training then
          ...
          if ply:KeyDown(IN_ATTACK2) then
            ...
            for k, v in pairs(ents.FindInSphere(b_pos, 20)) do
              if v:GetClass() == "billiard_cue" then
                b_bool = false
                break
              end
            end
...
concommand.Add("billiard_strike", function(ply, cmd, args)
  ...
  umsg.Start("billiard_shotView", ply)
  umsg.Bool(true)
  umsg.End()
  cue.ShotSpeed = speed
  cue.CanShoot = false
...
concommand.Add("billiard_acc_ref", function(ply, cmd, args)
  ...
  if tobool(args[1]) then
    ...
    umsg.String("Solicitud aceptada")
    umsg.String("Ya estas dentro de la partida.")
    umsg.End()
    ...
    umsg.Start("billiard_scrMsg", reqply)
    umsg.String("Solicitud rechazada")
    umsg.String("Tu solicitud ha sido rechazada.")
    umsg.End()
  end
end)
