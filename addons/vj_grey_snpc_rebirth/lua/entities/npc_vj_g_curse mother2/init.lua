...
if self:GetEnemy() ~= nil then
	if self.Cu3 == false and not IsValid(cu34) and not IsValid(cu44) and CurTime() > self.Cu3T then
	self.Cu3 = true

cu34 = ents.Create("npc_vj_g_curse")
cu34:SetPos(self:GetPos() + self:GetRight()*16 + self:GetUp()*50)
cu34:SetAngles(self:GetAngles())
cu34:Spawn()
cu34:SetColor(Color(0,0,0,110))
cu34:Activate()
cu34:SetOwner(self)
self:DeleteOnRemove(cu34)

cu44 = ents.Create("npc_vj_g_curse")
cu44:SetPos(self:GetPos() + self:GetRight()*-16 + self:GetUp()*30)
cu44:SetAngles(self:GetAngles())
cu44:Spawn()
cu44:SetColor(Color(0,0,0,110))
cu44:Activate()
cu44:SetOwner(self)
self:DeleteOnRemove(cu44)
	self.Cu3 = false
	self.Cu3T = CurTime() + math.random(5,13)
	end
	end
	end
--[[
